 ...
 * ---end---
 *
 * @author Shawn Crosby (sacrosby@gmail.com) - Initial contribution
 *
 */
//@NonNullByDefault
public class CloudOauthWinkAuthenticationService implements IWinkAuthenticationService {
    ...
    public CloudOauthWinkAuthenticationService(Map<String, String> properties) {
        ...
        try{
            pollingInterval = Integer.parseInt(properties.get("pollingInterval"));
        }catch (NumberFormatException e)
        {
            logger.error("CloudOauthWinkAuthenticationService failed to parse pollingIntervalSeconds to integer, defaulting to 300 seocnds. e: {}", e.getMessage());
            pollingInterval = 300;
        }
    }
